...
def build_customer_dataset(data_path, name, eval_scale, cache_images=False, no_image_reading=False, logger_prefix="OS2D"):
    logger = logging.getLogger(f"{logger_prefix}.dataset")
    logger.info("Preparing the GroZi-3.2k dataset: version {0}, eval scale {1}, image caching {2}".format(name, eval_scale, cache_images))

    annotation_folder="classes"
    image_size = 4000
    classdatafile = os.path.join(data_path, "customer", annotation_folder,"customer.csv")
    gt_path = os.path.join(data_path, "customer", annotation_folder, "images")
    image_path = os.path.join(data_path, "customer", "src", str(image_size))
    gtboxframe = read_annotation_file(classdatafile)

    # define a subset split (using closure)
    subset_name = name.lower()
    assert subset_name.startswith("customer"), ""
    subset_name = subset_name[len("customer"):]
    subsets = ["train", "val-old-cl", "val-new-cl", "val-all", "train-mini"]
    found_subset = False
    for subset in subsets:
        if subset_name == "-"+subset:
...
def build_dataset_by_name(data_path, name, eval_scale, cache_images=False, no_image_reading=False, logger_prefix="OS2D"):
    ...
    elif name.lower().startswith("customer"):
        return build_customer_dataset(data_path, name, eval_scale, cache_images=cache_images, no_image_reading=no_image_reading, logger_prefix=logger_prefix)
    else:
